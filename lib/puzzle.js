...
window.onload = function(){
	...
	container_width = document.getElementById("puzzle_area").scrollWidth;
	document.getElementById("puzzle_area").style.height = container_width + "px";
	init();
	setting();
}
function init(){
...
function createPuzzleBox(){
	var width = container_width / COLNUM;
	var height = width;
	...
	for( var i = 0; i < COLNUM * COLNUM; i++ ){
		...
		puzzle[i].style.backgroundSize = container_width + "px " + container_width + "px";
...
function canMove(obj){
	...
	//add a "1" to tolerent
	if (distance > container_width / COLNUM + 1){
		return false;
	}
	return true;
}
...
function checkWin(){
	for(var i = 0; i < puzzles.length; i++){
		//optimization method
		if((puzzles[i].offsetLeft - (i % COLNUM)*container_width/COLNUM ) > 1 || (puzzles[i].offsetTop - parseInt(i / COLNUM)*container_width/COLNUM) > 1 ){
			return false;
		}
	}
	return true;
